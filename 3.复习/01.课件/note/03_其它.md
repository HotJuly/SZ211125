...
- 3XX: 需要重定向, 浏览器直接跳转,例如:302(将当前请求的地址强行跳转到指定的网址),304(协商缓存,当用户请求资源的时候,服务器发现上次已经请求过了,会将当前请求重定向到浏览器的内存中)
- 4XX: 客户端请求错误,403(禁止访问,服务器具有该资源,但是你的权限不够,所以不给你),404(找到该资源,服务器返回没有该资源,大概率是前端请求路径错误)
- 5XX: 服务器端错误, 如: 500
...
- Restless API  
  ...
  - 传统的API, 把每个url当作一个功能操作 /getTradeMarkList	/deleteTradeMark
...
- **面试题:如何实现跨标签页通信?**
  - **前言:假设此时具有两个标签页,分别为A和B页面**
  - **方法一:使用localStorage实现跨标签页通信**
    - **在需要接收数据的B页面,给window绑定storage事件,用于监视浏览器对localStorage的修改**
    - **在需要发送数据的A页面,使用setItem存入数据,此时会触发B页面的storage事件的回调函数**
    - **注意:该方法只适用于同源的情况下**
  - **方法二:使用domain属性实现跨标签页通信**
    - **通过window.open方法打开A页面,该方法的返回值是A页面的window对象,将其保存起来**
    - **同时对A和B页面都进行domain属性的设置,保证两个页面数据互通**
      - **document.domain="共同的顶级域名"**
    - **A页面在window对象身上添加需要传递的数据**
    - **B页面在流程1中获取到的window对象身上读取对应属性即可**
    - **注意:该方法只适用于同一家公司的具有共同顶级域名的情况**
  - **方法三:可以通过页面跳转的时候,对url进行加工,实现路由传参**
    - **注意:该方法只适用于跳转页面的瞬间**
...
- layout 布局：计算元素的位置和大小信息(计算布局排版)
  - 在页面更新时,如果某个元素的宽高和定位等属性发生变化,可能会导致页面元素排版发生变化,就需要进行重排
- render 渲染：将颜色/文字/图片等渲染上去(绘制页面内容)
  - 在页面更新时,如果某些元素的样式,例如背景颜色,字体颜色,等发生变化,就会导致页面元素重新绘制
  - 注意:重排一定伴随着重绘,重绘不一定伴随着重排
