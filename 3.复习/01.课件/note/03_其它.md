...
1. 配置通用的基础路径和超时时间
   1. **超时时间就是当请求发出超过该时间还没有返回响应就会自动变为失败(404)**
      1. **小程序默认超时时间:60s**
      2. **axios配置为20s**
   2. **小程序的请求并发数为10个,Chrome和Firefox最多为6个**
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
- cookie
- sessionStorage
  - 刷新当前页面,sessionStorage数据不会丢失
  - 复制当前页面,sessionStorage数据不会丢失
  - 复制当前页面地址,然后打开新的标签页,输入地址跳转该页面,sessionStorage数据会丢失
- localStorage
...
- **问题:如何跨标签页通信?**
  - **前言:发送数据的为A页面,接收数据的为B页面**
  - **方法一:使用localStorage存储并传递数据**
    - 流程: 
      - **1.B页面需要做好接收数据的准备,监视storage是否具有变化**
        - **在window对象身上.绑定storage事件,可以监视storage是否发生变化**
      - **2.A页面将数据存入到localStorage中**
      - **3.B页面的监听就会触发,从事件对象event中可以获取到本次的最新key值和value值**
    - **使用约束:必须A和B页面的域名必须相同**
  - **方法二:修改domain属性,实现window对象共享**
    - **流程:**
      - **1.B页面将自己的document.domain属性修改为共同的顶级域名**
      - **2.B页面通过window.open方法打开A页面,从而获取到A页面的window对象**
        - **window.open的返回值就是被打开页面的window对象**
      - **3.A页面将数据存储到自身的window对象上**
      - **4.A页面将自己的document.domain属性修改为共同的顶级域名**
      - **5.B页面可以直接通过流程2中获取到的window对象,直接读取数据**
    - **使用约束:A和B页面必须拥有共同的顶级域名**
  - **方法三:通过跳转时候,在url中拼接参数进行传递**
    - **使用约束:只有在跳转的时候才能传递一次数据**
...
1. DNS 解析（查询）：(将域名解析变成对应的IP地址)
   1. 域名的好处就是方便记录,缺点会导致页面加载速度变慢
...
- layout 布局：计算元素的位置和大小信息
  - 如果后续页面更新的时候,元素的大小或位置发生变化,页面会发生重排(重新布局)
- render 渲染：将颜色/文字/图片等渲染上去
  - 如果后续页面更新的时候,元素的颜色图片等发生变化,页面会发生重绘(重新渲染)
  - 总结:重排一定伴随着重绘,重绘不一定伴随着重排
  - 扩展:如何给一个元素开启图层
    - 1.video,canvas标签自带图层
    - 2.使用C3的大多数新特性都自带图层
    - 3.添加css样式->will-change:auto
